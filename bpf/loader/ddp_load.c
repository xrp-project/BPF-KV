...
static void load_and_attach_program(void)
{
	...
	ret = bpf_ddp_load(&obj, cfg_path_name, cfg_section_name, &prog_fd);
	if (ret)
		error(1, 0, "bpf_ddp_load %s", cfg_path_name);

	ret = bpf_prog_attach(prog_fd, 0 /* Ignore */, BPF_DDP, 0);
	if (ret)
